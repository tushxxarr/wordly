...
async def load_neural_dictionary():
    """Downloads dictionary in the background to prevent blocking Render's startup."""
...
async def web_server_handler(request):
    return web.Response(text="Lexicore Neural Word Engine is Operational ðŸŒŒðŸš€")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == constants.ChatType.PRIVATE and LOG_GROUP_ID:
        text = f"ðŸ‘¤ **New User in PM**\nName: {user.first_name}\nID: `{user.id}`\nUsername: @{user.username}"
        try:
            await context.bot.send_message(chat_id=LOG_GROUP_ID, text=text, parse_mode=constants.ParseMode.MARKDOWN)
        except Exception: pass

    welcome = (
        f"ðŸŒŒ **Welcome to Lexicore, {user.first_name}!** ðŸŒŒ\n\n"
        "The universe's fastest, most advanced anagram-scramble bot.\n\n"
        "ðŸŽ® `/new` â€” Begin Scramble Game (Easy Mode)\n"
        "ðŸ’¥ `/hard` â€” Begin Scramble Game (Hard Mode)\n"
        "ðŸ“– `/help` â€” Rules & Scoring\n\n"
        "Add me to your group and make me admin to ignite the competition! ðŸš€"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "âš½  **Scoring System** âš½\n"
        "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"
        "ðŸ”¸ 3 letters: 1 point\n"
        "ðŸ”¸ 4 letters: 2 points\n"
        "ðŸ”¸ 5 letters: 3 points\n"
        "ðŸ”¸ 6 letters: 5 points\n"
        "ðŸ”¸ 7 letters: 7 points\n"
        "ðŸŽ¯ 8 to 12 letters: 10 points\n\n"
...
async def start_game_easy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not NEURAL_DICTIONARY:
        await update.message.reply_text("â³ Dictionary is still loading. Please try again in a few seconds.")
        return

    if chat_id in active_games and active_games[chat_id].active:
        await update.message.reply_text("âš ï¸ Scramble already in progress. Finish it or use `/stop`.")
        return
    ...
    game_start_text = (
        f"ðŸŒŒ **New Scramble: EASY MODE** ðŸŒŒ\n\n"
        f"ðŸ”Ž Make words using these letters:\n\n"
        f"ðŸ”¤ ** {game.scramble} **\n\n"
        f"Bounds: 3-12 letters âœ…\n"
        f"Find {game.max_rounds} words for results! ðŸŽ¯"
    )
...
async def start_game_hard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game_start_text = (
        f"ðŸ’¥ **New Scramble: HARD MODE** ðŸ’¥\n\n"
        f"ðŸ”Ž Make words using these letters:\n\n"
        f"ðŸ”¤ ** {game.scramble} **\n\n"
        f"Bounds: 4-12 letters (+5 pts each) âœ…\n"
        f"Find {game.max_rounds} words for results! ðŸŽ¯"
    )
    await update.message.reply_text(game_start_text, parse_mode=constants.ParseMode.MARKDOWN)
...
async def stop_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in active_games and active_games[chat_id].active:
        ...
        results = results.replace("ðŸŽ‰ **Game Over** ðŸŽ‰", "ðŸ›‘ **Sequence Aborted**")
        ...
        await update.message.reply_text("âš ï¸ No active sequence to stop.")
...
async def check_guesses(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    score_increment = f"+{points} ðŸ’Ž"
    results_update = (
        f"âœ… {user.first_name} found `{guess.capitalize()}`!\n"
        f"{score_increment}\n\n"
...
async def log_joining_groups_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_status in [ChatMember.MEMBER, ChatMember.ADMINISTRATOR] and old_status in [ChatMember.LEFT, ChatMember.BANNED]:
        welcome = (
            f"ðŸ‘‹ **Lexicore joined {chat.title}!** ðŸŒŒ\n\n"
            "âš ï¸ **Please promote me to Admin** so I can read guesses and update results.\n\n"
    ...
    if new_status == ChatMember.ADMINISTRATOR and old_status != ChatMember.ADMINISTRATOR and LOG_GROUP_ID:
        try:
            ...
            log_text = (
                f"ðŸš¨ **NEW GROUP ADMIN DOSSIER** ðŸš¨\n\n"
                f"**Group:** {chat.title}\n"
                f"**Group ID:** `{chat.id}`\n"
...
async def sudo_only_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command_text.startswith('/add'):
        args = context.args
        if len(args) == 0:
            await update.message.reply_text("âš ï¸ Usage: `/add <word>`")
            return
        
        suggested_word = args[0].lower()
        if LOG_GROUP_ID:
            log_text = f"ðŸ†• **Word Suggestion**\nWord: `{suggested_word}`\nBy: @{user.username} (`{user.id}`)"
            await context.bot.send_message(chat_id=LOG_GROUP_ID, text=log_text, parse_mode=constants.ParseMode.MARKDOWN)
        await update.message.reply_text(f"âœ… Suggestion for `{suggested_word}` submitted for review.")
...
def main():
    ...
    print("ðŸš€ Lexicore Engine is ready.")
